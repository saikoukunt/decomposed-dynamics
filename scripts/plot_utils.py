    ...
    if len(trial_ids) == 0:
        ...
        for i in range(len(unique_trial_ids)):
            ...
            if trial_ids[first_idx] != unique_trial_ids[i]:
                first_idx = -1
                print(
                    f"Warning: trial id {unique_trial_ids[i]} not found in trial_ids array."
                )
            rep_trial_ids = np.append(rep_trial_ids, first_idx)

        C_averaged = {
            l: np.zeros_like(C[trial_keys[int(rep_trial_ids[int(l)])]])
            for l in unique_trial_ids
        }
...
def plot_Cs_one_F(C, F_idx, trial_ids: np.array = []):
    ...
    for k in range(len(trial_ids)):
        if C_forF[int(trial_ids[k])].size == 0:
            C_forF[int(trial_ids[k])] = np.atleast_2d(
                np.array(C[trial_keys[k]][0, F_idx, :].squeeze())
            )
        else:
            C_forF[int(trial_ids[k])] = np.append(
                np.atleast_2d(C_forF[int(trial_ids[k])]),
                np.atleast_2d(np.array(C[trial_keys[k]][0, F_idx, :].squeeze())),
                axis=0,
            )
    ...
    for i, trial_id in enumerate(trial_ids):
        ...
        if len(C_hat_repackaged[int(trial_id)]) == 0:
            ...
            C_hat_repackaged[int(trial_id)] = np.append(
                ...
                np.atleast_3d(np.array(Ctmp.squeeze())),
                axis=2,
            )
